...
模块的命名不能有-，可以用\_代替。\
reload\_all		#从目录重载所有模块。导入模块也需要用到此命令。
...
参数选项：
-p			指定的payload
-e 			编码器，如 x86/shikata_ga_nai（仅适用于 x86 payload，x64 payload 不支持）
-i			迭代器，对有效载荷的编码次数
...
ps				#获取目标机正运行的进程
migrate 476		#将shell迁移到PID为786的进程中（32位会话可迁到64位进程，若目标进程是SYSTEM则权限继承过来）
```
...
关于注册表，有必要提一下HKLM。HKLM是 Windows 操作系统注册表中的一个根键，它代表了 HKEY\_LOCAL\_MACHINE，其中 "HKEY" 表示 "Handle to a Key"（键的句柄），而 "LOCAL\_MACHINE" 表示本地计算机的配置信息。HKLM 包含了计算机的硬件和操作系统的配置信息、安装的软件信息、用户和组的安全设置等等。
...
添加完成后可以考虑使用ms17\_010的扫描脚本进行目标内网的扫描
